...
export const Content = ()=> {
    ...
    return(
        <div className={`flex-1 ${imcCtx?.imc !== 0 ? 'bg-white/20': ''}`}>
            ...
            <input 
            type="number"
            className="spin outline-none text-sm sm:text-lg bg-none border-b p-3 border-zinc-300 w-full mb-4 sm:mb-8 text-zinc-500 hover:border-blue-400 hover:text-blue-400 hover:placeholder:text-blue-400 focus:border-blue-400 transition-all" 
            placeholder="Digite o seu peso. Ex: 47.7 (em kg)"
            onChange={e=>setWeightInput(e.target.value)}
            value={weightInput}
            />
            <button onClick={   handleCalcButton} className="w-full py-3 text-white bg-blue-400 rounded-lg mt-4 sm:mt-10 hover:bg-blue-500 transition-all">Calcular</button>
        </div>
    )
}